...
AudioMixer::AudioMixer() : AudioBuffer() {
...
int AudioMixer::read_callback(void *rwops,uint8_t *buf,int size){
	return ((RWops*)rwops)->Read(buf,size);
}

// Implement writing from blobs
int AudioMixer::write_callback(void *rwops,uint8_t *buf,int size){
	return ((RWops*)rwops)->Write(buf,size);
}

// Implement seeking from blobs
int64_t AudioMixer::seek_callback(void *rwops,int64_t offset,int whence){
...
void AudioMixer::log_callback(void *ptr,int level,const char* fmt,va_list vl){
	const int size=255;
...
bool AudioMixer::Seek(uint64_t Offset){
	bool retval=false;
	SDL_LockMutex(datalock);
	if(FormatContext){
 ...
 */
void AudioMixer::Mix(unsigned char *Output,int Length,double Volume){
	SDL_LockMutex(framelock);
 ...
 */
bool AudioMixer::Open(RWops *Resource){
