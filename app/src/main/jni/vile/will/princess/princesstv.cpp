...
PrincessTextview::PrincessTextview(PrincessWaltz *Engine) : Textview(Engine) {
	SDL_Rect rect;
	...
	if(winbase){
		...
		for(int i=0;winbase[i];i++){
			if(Engine->GetImagePosition("winbase0",i,&rect)){
				...
				if(i==PRINCESSTV_BUTTON_HISTORY){
					// Configure widgets
					rect.y+=ypos;
...
PrincessTextview::~PrincessTextview(){
}

bool PrincessTextview::InputOk(Widget *Object){
	bool retval=false;
	if(Object==w_load){
		((PrincessWaltz*)engine)->EventLoad(998);
		retval=true;
	}
	else if(Object==w_save){
		((PrincessWaltz*)engine)->EventSave(998);
		w_save->SetValue(true);
		retval=true;
	}
	else if(Object==w_history){
		((PrincessWaltz*)engine)->EventGameDialog(VD_LOG);
	}
	else if(Object==w_skip){
		((PrincessWaltz*)engine)->SetSkipmode(true);
...
void PrincessTextview::PrintText(uString Title,uString Text){
	if(header){
		header->Clear();
...
void PrincessTextview::PrintText(uString Text){
	if(header){
