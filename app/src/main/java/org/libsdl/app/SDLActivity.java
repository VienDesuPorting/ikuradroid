...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    // ------------------------------------------------------------------
    // IkuraDroid customization.
    //
    // The library screen passes the game folder and the savegame root
    // as intent extras. The folder becomes the process working directory
    // (nativeInit -> chdir, see jni/sdl_main/sdl_main.c), so the engine
    // keeps opening its archives with plain relative paths; the save
    // root travels to SDL_main as the "--save" argument (getArguments).
    public static String gCurrentDirectoryPath;
    public static String gCurrentSavePath;

    // Auto-skip toggle: the engine holds CTRL while the user reads the
    // in-game menu's skip entry; any touch releases it (SDLSurface).
    ...
    // one - and simply never delivers touches that start there; if the
    // system claims a pull midway, the stream ends in ACTION_CANCEL and
    // the tracker drops it without any action. The touch is captured at
    // ACTION_DOWN (the engine must not see it: a background down
    // advances the text, so a leaked swipe start would skip a line) and
    // released as soon as the finger proves it is not a vertical
    // gesture: within MENU_SWIPE_SLOP_DP it is replayed as one clean tap
    // once the finger lifts; past the slop in a non-vertical direction
    // the stream is handed back to the surface from the current
    // position, so drag-hover keeps working. An upward run of at least
...
*/
    ...
    protected String getMainSharedObject() {
        // SDL_main (the ViLE engine entry point) lives in libikuradroid.so;
        // see jni/vile/vile.cpp (main is renamed by SDL_main.h).
        return getContext().getApplicationInfo().nativeLibraryDir + "/libikuradroid.so";
    }
    ...
    protected String[] getLibraries() {
        // Same set and order the 1.x build used in its static
        // initializer: dependencies before their consumers, the launch
        // glue (sdl_main) last.
        return new String[] {
            "SDL2",
            "SDL2_image",
    ...
    protected String[] getArguments() {
        // The engine parses "--save FOLDER" (vile.cpp); the working
        // directory needs no argument - nativeInit chdir'ed already.
        return new String[] { "--save", gCurrentSavePath };
    }
    ...
    public static void initialize() {
        ...
        // IkuraDroid: the skip toggle must not leak into the next session
        skip = false;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // IkuraDroid: resolve the game folder and the savegame root from
        // the launch intent (see MainActivity.launchGame), chdir into the
        // folder natively. Must run before the SDL thread starts.
        Intent launchIntent = getIntent();
        String fName = launchIntent.getStringExtra("fname");
        String fPath = launchIntent.getStringExtra("fpath");
        gCurrentDirectoryPath = fPath + "/" + fName;
        gCurrentSavePath = getExternalFilesDir(null).getAbsolutePath();
        nativeInit(gCurrentDirectoryPath, gCurrentSavePath);
    ...
    private void showGameMenu() {
        // Never stack a second sheet over a live one: the dialog is
        // tracked as a field, so repeated triggers cannot pile up
        // identical copies.
     ...
     * which stops the SDL thread cleanly. The engine never sees the
     * F9/VD_SHUTDOWN path, so its own "Exit game?" prompt
    ...
    //    ACTION_CANCEL, which drops the tracker without any action;
    // ------------------------------------------------------------------
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (!mSwipeTracking) {
            if (action == MotionEvent.ACTION_DOWN) {
                // Capture from anywhere, edges included (see the comment
                // block above): the system zones keep their own touches,
                // everything else is a potential gesture.
                mSwipeTracking = true;
    ...
    protected void onPause() {
        ...
        if (!mHasMultiWindow) {
            // IkuraDroid: silence the engine mixer together with the
            // native pause (Mix_Pause*, see emixer.cpp)
            if (!SDLActivity.mBrokenLibraries) {
                pauseMixer();
            }
            pauseNativeThread();
    ...
    protected void onDestroy() {
        ...
        // A live bottom sheet would leak its window across the finish
        // (engine-initiated exits can land here while the menu shows)
        if (mGameMenuSheet != null) {
            if (mGameMenuSheet.isShowing()) {
                mGameMenuSheet.dismiss();
            }
            mGameMenuSheet = null;
        }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        // IkuraDroid: BACK and MENU open the Material 3 in-game menu; the
        // event is consumed so the engine never sees it. NB:
        // dispatchKeyEvent sees BOTH the ACTION_DOWN and the ACTION_UP of
        // a single physical press - the sheet must show exactly once, on
    ...
    // IkuraDroid natives: the launch glue (jni/sdl_main) and the engine
    // mixer (jni/vile/engine/emixer.cpp)
    public static native void nativeInit(String currentDirectoryPath, String currentSavePath);
    public static native void pauseMixer();
